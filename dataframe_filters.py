...
def filter_by_launch_category(dataset_class, launch_categories):
    ...
    if (type(dataset_class) == mcdowell_launch.McDowellLaunch):
        raise ValueError("launch dataset class expected by filter_by_launch_category(). Cannot sort by launch category in satcat dataset.")
...
def filter_by_launch_success_fraction(dataset_class, launch_success_fractions):
    ...
    if (type(dataset_class) == mcdowell_satcat.McDowellSatcat):
        raise ValueError("launch dataset class expected by filter_by_launch_success_fraction(). Cannot sort by launch success fraction in satcat dataset.")
...
def filter_by_sat_type_coarse(dataset_class, sat_types):
    ...
    if (type(dataset_class) == mcdowell_launch.McDowellLaunch):
        raise ValueError("satcat dataset class expected by filter_by_sat_type_coarse(). Cannot sort by sat type in launch dataset.")
...
def filter_by_separation_date(dataset_class, start_date=None, end_date=None):
    ...
    if (type(dataset_class) == mcdowell_launch.McDowellLaunch):
        raise ValueError("satcat dataset class expected by filter_by_separation_date(). Cannot sort by separation date in launch dataset.")
...
def filter_by_decay_date(dataset_class, start_date=None, end_date=None):
    ...
    if (type(dataset_class) == mcdowell_launch.McDowellLaunch):
        raise ValueError("satcat dataset class expected by filter_by_decay_date(). Сannot sort by decay date in launch dataset.")
