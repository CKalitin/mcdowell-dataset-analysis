...
class McDowellDataset:
    """
    This contains all functions required for using McDowell's dataset. This includes filtering the dataset by year, etc.
    """
    ...
    def __init__(self, file_path="datasets/launch.tsv"):
        ...
        self.launch_df["pandas_date"] = pd.to_datetime(self.launch_df["Launch_Date"], errors="coerce")
    ...
    def filter_by_date(self, start_date=None, end_date=None):
        ...
        if start_date is not None:
            self.launch_df = self.launch_df[self.launch_df["pandas_date"] >= start_date]
        
        if end_date is not None:
            self.launch_df = self.launch_df[self.launch_df["pandas_date"] <= end_date]


pd.set_option('display.max_columns', None)
        
dataset = McDowellDataset()
